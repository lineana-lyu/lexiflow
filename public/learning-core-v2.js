(() => {
  ...
  function crossDayPatch(prev, next, now = new Date()){
    ...
    if(prev.stage === "apply" && next.stage === "review"){
      return {stageEligibleOn:addDaysIso(now,1),applyCompletedOn:today,memoryState:"reinforcing",reviewStep:0,nextReviewAt:addDaysIso(now,1),initialReviewPending:true};
    }
    return null;
  }
  ...
  function reviewSchedulePatch(prev, quality, now = new Date()){
    ...
    if(prev.memoryState === "review_again"){
      ...
      if(sameDay){
        return {memoryState:"review_again",reviewAgainNeedsNextDay:true,nextReviewAt:addDaysIso(now,1)};
      }
      if(quality === "good"){
        ...
        return {memoryState:"reinforcing",reviewStep:recovered,reviewAgainNeedsNextDay:false,nextReviewAt:addDaysIso(now,REVIEW_INTERVALS[recovered])};
      }
      return {memoryState:"review_again",reviewAgainFailedOn:today,sameDayRetestUsedOn:today,reviewAgainNeedsNextDay:true,nextReviewAt:now.toISOString()};
    }

    if(quality !== "good"){
      return {memoryState:"review_again",reviewAgainOriginStep:Number(prev.reviewStep||0),reviewAgainFailedOn:today,sameDayRetestUsedOn:today,reviewAgainNeedsNextDay:true,nextReviewAt:now.toISOString()};
    }
