(() => {
  ...
  if(!core) throw new Error("LexiFlowLearningCore must load before today-plan-v3.js");
  ...
  async function refresh(){
    try{
      ...
      console.warn("Today Plan V3 refresh failed",err);
    }
    return latestData;
  }
  ...
  async function moveBackToPending(id){
    ...
    try{
      ...
      if(!card||card.stage!=="select"||card.inboxPending)return false;
      ...
      next.activities=Array.isArray(next.activities)?next.activities:[];next.activities.push({id:uid(),type:"library-unselected",cardId:card.id,at:now.toISOString(),authority:"today-plan-v3"});
      await persist(next,"library-unselected");location.reload();return true;
    }finally{saving=false;}
  }
