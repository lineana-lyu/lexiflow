...
(() => {
  ...
  function renderLookupResult(){
    ...
    return `${resolvedNote}${alternativePanel}
      ...
      <div data-etymology-host="lookup" data-etymology-word="${escapeHtml(r.word)}" data-etymology-meaning="${escapeHtml(primarySense?.meaningZh||"")}"></div>
  ...
  function studyPage(){
    ...
    return shell(
      ...
      + `<div class="study-progress-wrap">${renderStageRail(card)}</div><div class="study-shell study-shell-single"><div class="study-depth-shell"><span class="study-stack-layer study-stack-layer-far" aria-hidden="true"></span><span class="study-stack-layer study-stack-layer-near" aria-hidden="true"></span><div class="card study-card study-card-focus">${renderStage(card)}</div></div><div data-etymology-host="study" data-etymology-word="${escapeHtml(card.word)}" data-etymology-meaning="${escapeHtml(card.meaningZh||"")}"></div></div>`
    );
  }
  ...
  function normalizeWordFamily(groups){
    if(!Array.isArray(groups))return [];
    const allowed=new Set(["noun","verb","adjective"]);
    return groups.map(group=>{
      const pos=String(group?.pos||"").trim().toLowerCase();
      const label=String(group?.label||({noun:"名词",verb:"动词",adjective:"形容词"}[pos]||"")).trim();
      const seen=new Set();
      const words=(Array.isArray(group?.words)?group.words:[])
        ...
        .slice(0,3);
      return allowed.has(pos)&&label&&words.length?{pos,label,words}:null;
    }).filter(Boolean);
  }

  function wordFamilyMarkup(groups,{compact=false}={}){
    const items=normalizeWordFamily(groups);
    if(!items.length)return "";
    return `<div class="lexi-word-family ${compact?"compact":""}" aria-label="词性家族">
      <div class="lexi-word-family-title"><span>词性家族</span><small>同一词族的名词 · 动词 · 形容词</small></div>
      <div class="lexi-word-family-list">${items.map(group=>`<div class="lexi-word-family-row"><strong>${escapeHtml(group.label)}</strong><div>${group.words.map(word=>`<span>${escapeHtml(word)}</span>`).join("")}</div></div>`).join("")}</div>
    </div>`;
  }

  async function ensureCardWordFamily(card){
    if(!card||isMultiWordExpression(card.word)||normalizeWordFamily(card.wordFamily).length)return;
    const id=String(card.id||"");if(!id||state.wordFamilyHydration[id])return;
    state.wordFamilyHydration[id]="loading";
    try{
      const payload=await api("/api/dictionary/lookup",{method:"POST",body:{word:card.word,mode:"primary"}});
      const family=normalizeWordFamily(payload?.result?.wordFamily);
      if(family.length){
        card.wordFamily=family;
        card.updatedAt=new Date().toISOString();
        if(state.libraryEditor&&String(state.libraryEditor.cardId)===id){
          state.libraryEditor.draft={...(state.libraryEditor.draft||libraryEditorBaseDraft(card)),wordFamily:family};
        }
        saveData();
        state.wordFamilyHydration[id]="done";
        render();
        return;
      }
      state.wordFamilyHydration[id]="empty";
    }catch{
      state.wordFamilyHydration[id]="failed";
    }
  }
  ...
  function libraryEditorBaseDraft(card){
    return {
      ...
      wordFamily:normalizeWordFamily(card.wordFamily),
  ...
  function openLibraryEditor(cardId){
    ...
    render();
    void ensureCardWordFamily(card);
  }
    ...
    return shell(
      ...
      + `<div class="library-editor-grid library-editor-grid-refined">
        <section class="card library-editor-identity" aria-label="固定词条信息">
          ...
          ${wordFamilyMarkup(d.wordFamily,{compact:true})}
        </section>
