(() => {
  ...
  if(!core)throw new Error("LexiFlowLearningCore must load before advance-learning-v2.js");
  ...
  async function persist(next){
    const response=await fetch("/api/learning-data",{
      ...
      body:JSON.stringify({data:core.normalizeData(next)}),
    });
  ...
  function decorate(){
    ...
    if(existing)existing.replaceWith(box);
      ...
      const inbox=planRoot.querySelector(".lexi-inbox");
      if(inbox)inbox.insertAdjacentElement("beforebegin",box);else planRoot.appendChild(box);
    }
  }
