(() => {
  ...
  async function persistSettings(patch){
    ...
    saving=true;
    try{
      await refresh();if(!latestData)return;
      const next=clone(latestData);next.settings=normalizeSettings({...next.settings,...patch});
      const response=await previousFetch("/api/learning-data",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({data:next})});
      if(!response.ok)throw new Error("SAVE_FAILED");
      latestData=next;schedule();
    }catch(err){console.error("review policy save failed",err);}finally{saving=false;}
  }

  function injectStyle(){
    if(document.getElementById("lexi-review-policy-style"))return;
    const style=document.createElement("style");style.id="lexi-review-policy-style";
    style.textContent=`
  ...
  function settingsSignature(settings){
    ...
    return JSON.stringify({mode:s.reviewMode,cap:s.reviewCustomCap,types:s.reviewTypes,weights:s.reviewTypeWeights,advanced:advancedOpen});
  }

  function settingsHtml(settings,signature){
    ...
    return `<div class="setting-row lexi-review-policy-row" data-review-policy-row data-review-policy-signature="${String(signature).replace(/&/g,"&amp;").replace(/"/g,"&quot;")}">
  ...
  function decorateSettings(){
    const list=settingsPage();if(!list||!latestData)return;
    const settings=normalizeSettings(latestData.settings||{}), signature=settingsSignature(settings);
    let row=list.querySelector("[data-review-policy-row]");
    if(row?.dataset.reviewPolicySignature===signature)return;
    const holder=document.createElement("div");holder.innerHTML=settingsHtml(settings,signature);const next=holder.firstElementChild;
  ...
  function decorateToday(){
    ...
    const plan=latestData.dailyPlan, reviewRow=document.querySelector('#lexi-today-plan [data-plan-key="review"]');
    if(!reviewRow||!plan)return;
    const count=reviewRow.querySelector(".lexi-plan-count"), scheduled=Number(plan.review?.length||0), total=Number(plan.reviewDueTotal??scheduled), deferred=Math.max(0,Number(plan.reviewDeferredCount||0));
    if(count)count.title=deferred>0?`今天安排 ${scheduled} 个；另有 ${deferred} 个到期词会在之后的学习日重新计算。`:total?`今天安排 ${scheduled} 个到期词。`:"今天没有到期复习。";
  }

  document.addEventListener("click",event=>{
    const mode=event.target?.closest?.("[data-review-mode]");if(mode){event.preventDefault();void persistSettings({reviewMode:cleanMode(mode.dataset.reviewMode)});return;}
    const advanced=event.target?.closest?.("[data-review-advanced]");if(advanced){event.preventDefault();advancedOpen=!advancedOpen;decorateSettings();return;}
    const reset=event.target?.closest?.("[data-review-reset]");if(reset){event.preventDefault();void persistSettings({reviewTypes:{...DEFAULT_TYPES},reviewTypeWeights:{...DEFAULT_WEIGHTS}});return;}
  },true);

  document.addEventListener("change",event=>{
    const target=event.target;if(!(target instanceof HTMLInputElement))return;
    if(target.id==="review-custom-cap"){const value=Math.max(1,Math.min(200,Math.round(Number(target.value)||20)));target.value=String(value);void persistSettings({reviewCustomCap:value});return;}
    if(target.dataset.reviewType){const settings=normalizeSettings(latestData?.settings||{}), next={...settings.reviewTypes,[target.dataset.reviewType]:target.checked};if(!Object.values(next).some(Boolean)){target.checked=true;return;}void persistSettings({reviewTypes:next});return;}
    if(target.dataset.reviewWeight){const settings=normalizeSettings(latestData?.settings||{}), value=Math.max(1,Math.min(100,Math.round(Number(target.value)||DEFAULT_WEIGHTS[target.dataset.reviewWeight]||20)));target.value=String(value);void persistSettings({reviewTypeWeights:{...settings.reviewTypeWeights,[target.dataset.reviewWeight]:value}});}
  },true);
