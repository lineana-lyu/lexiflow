(() => {
  ...
  async function loadData(){const response=await fetch("/api/learning-data",{cache:"no-store"});if(!response.ok)throw new Error("LOAD_FAILED");const payload=await response.json();return payload?.data?core.normalizeData(payload.data):null;}
  async function persist(data){const response=await fetch("/api/learning-data",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({data:core.normalizeData(data)})});if(!response.ok)throw new Error("SAVE_FAILED");}
  ...
  function appendActivity(data,cardId,stage,extra={}){data.activities=Array.isArray(data.activities)?data.activities:[];if(extra.commandId&&commandCommitted(data,extra.commandId))return;data.activities.push({id:uid(),type:"stage-complete",cardId,stage,at:new Date().toISOString(),...extra});}
