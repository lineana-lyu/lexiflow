(() => {
  ...
  function targetForms(word){
    ...
    const irregular = {
      keep:["kept"], run:["ran","running"], write:["wrote","written","writing"],
      go:["went","gone"], have:["has","had"], do:["does","did","done"], make:["made"],
      take:["took","taken"], see:["saw","seen"], come:["came"], get:["got","gotten"],
      give:["gave","given"], eat:["ate","eaten"], buy:["bought"], bring:["brought"],
  ...
  function domWord(){
    return normalize(document.querySelector(
      ".study-card-focus .target-word-text,.apply-word-hero .target-word-text,.apply-word-hero strong,.word-title"
    )?.textContent || "");
  }
  ...
  async function completeSelect(button){
    ...
    Object.assign(card, core.crossDayPatch({stage:"select"},{stage:"memorize1"},now) || {});
  ...
  async function completeVisualize(button){
    ...
    card.stage = "apply";
    Object.assign(card, core.crossDayPatch({stage:"visualize"},{stage:"apply"},now) || {});
    card.updatedAt = now.toISOString();
  ...
  async function completeApply(button){
    ...
    Object.assign(card, core.crossDayPatch({stage:"apply"},{stage:"review"},now) || {});
    card.updatedAt = now.toISOString();
  ...
  async function handle(button, kind){
    ...
    try{
      ...
      if(button){
        ...
        delete button.dataset.lexiTransitionSaving;
      }
    }finally{
      saving = false;
    }
  ...
  // Register this capture listener before legacy app handlers. These three transitions
  // are now authoritative here, so the old app.js stage mutation functions only render UI.
  document.addEventListener("click",event=>{
    ...
    if(button.matches('[data-action="finish-visual"]')){
      ...
      event.preventDefault();
      event.stopImmediatePropagation();
      void handle(button,"visualize");
      return;
    }

    if(button.matches('[data-action="pass-apply"]')){
      const text = String(document.getElementById("apply-text")?.value || "").trim();
      const word = domWord();
      if(!text || containsChinese(text) || !usesTarget(text,word)) return;
      event.preventDefault();
      event.stopImmediatePropagation();
      void handle(button,"apply");
    }
  },true);
})();