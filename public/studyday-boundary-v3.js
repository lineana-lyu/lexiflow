(() => {
  ...
  if(!core)throw new Error("LexiFlowLearningCore must load before studyday-boundary-v3.js");

  const RUNTIME_KEY="lexiflow-studyday-runtime-v3";
  const LEGACY_RUNTIME_KEY="lexiflow-studyday-runtime-v2";
  ...
  function storedRuntimeDay(){
    try{return String(localStorage.getItem(RUNTIME_KEY)||localStorage.getItem(LEGACY_RUNTIME_KEY)||"");}catch{return"";}
  }
  function storeRuntimeDay(day){
    try{
      localStorage.setItem(RUNTIME_KEY,day);
      localStorage.removeItem(LEGACY_RUNTIME_KEY);
    }catch{}
  }
  ...
  function checkBoundary(){
    ...
    if(current===runtimeDay)return false;
    runtimeDay=current;
  ...
  window.LexiFlowStudyDayBoundaryV3=Object.freeze({check:checkBoundary,currentDay:()=>runtimeDay});
})();
