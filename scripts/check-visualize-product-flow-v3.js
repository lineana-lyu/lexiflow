...
assert(server.includes("用户原始联想（最高优先级，必须保留）")&&server.includes("不能换人物、换地点、换故事"),"Server prompt must anchor on the user's own scene and forbid story substitution");
...
assert(guard.includes("isLegacyResidue")&&visual.includes("isLegacyDraftResidue"),"Visual input must discard obvious repeated-letter test residue instead of restoring it as a default scene");
