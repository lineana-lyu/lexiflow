...
replaceOnce(
  "server-runtime.js",
  String.raw`    if (queryKind === "word" && /^[A-Za-z][A-Za-z\s'-]*$/.test(query)) {
...
replaceOnce(
  "server-runtime.js",
  String.raw`    const queryKind = expressionQuery.classifyEnglishQuery(word);
...
replaceOnce(
  ...
  String.raw`  const queryKind = expressionQuery.classifyEnglishQuery(word);
  // Multi-word expressions must never inherit a single component's local ECDICT
  // phonetic/audio. Whole-phrase exact dictionary audio is resolved remotely;
  // otherwise the client synthesizes the complete expression.
...
replaceOnce(
  "server-runtime.js",
  String.raw`    const pronunciation = remote.payload?.result;
...
replaceOnce(
  ...
  String.raw`  width:38px!important;
...
if (!check.includes("multiword search must not short-circuit") || !check.includes("reject component recordings")) {
  throw new Error("expression regression checks were not updated");
}
