...
assert(apply.includes('handleInput(input,options){handleComposerInput(input,options);}')&&apply.includes('LexiFlowApplyInputGuardV3?.setDraft?.(card.id,s.text)'),"Apply stage must share draft authority with the pre-app guard");
