...
assert(server.includes("if (!feedback.issues.length && feedback.suggestion && feedback.changes.length)")&&server.includes("const fallbackBlocking = feedback.approved === false || feedback.level !== \"good\";")&&!server.includes('if (feedback.level === "good" && !feedback.suggestion) feedback.issues = [];'),"optional suggestions must survive normalization and fallback issues must inherit blocking semantics");
