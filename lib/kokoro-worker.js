...
function serializeError(err) {
  return {
    message: clean(err?.message || err) || "Kokoro process failed",
...
process.on("message", message => {
  ...
  else task = Promise.reject(Object.assign(new Error(`unknown Kokoro process op: ${op}`), { code: "KOKORO_BAD_OP" }));

  task.then(result => {
    send({ type: "result", id, ok: true, result });
  }).catch(err => {
    send({ type: "result", id, ok: false, error: serializeError(err) });
  });
});
