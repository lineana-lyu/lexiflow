...
function chineseCandidateBaseScore(item) {
...
function chineseCandidates(query, limit = 5) {
  ...
  try {
    const rows = db.prepare(`
      ...
      LIMIT ?
    `).all(q, Math.max(24, safeLimit * 4));
    return rows
      .map(item => ({ ...item, learning_score: chineseCandidateBaseScore(item) }))
...
function lookupChinese(query) {
  ...
  const resolved = candidates.map(item => {
    ...
    if (!result) return null;
    const semanticBoost = result.chineseSenseMatched ? 900 : 0;
    return { item, result, effectiveRank: chineseCandidateBaseScore(item) + semanticBoost };
