...
const LOOKUP_TERM_RE = /^-?[\p{L}][\p{L}\p{M}'’\-]{0,63}$/iu;
...
function decodeRevisionSource(payload){
  const page=mediaWikiPages(payload)[0];
  if(!page)return {state:"invalid",text:""};
  if(page.missing===true || page.missing==="")return {state:"missing",text:""};
...
function decodeParseSource(payload){
  if(payload?.error?.code==="missingtitle")return {state:"missing",text:""};
  const text=firstText([
...
function wiktionarySourceStrategies(word){
  const common={
    format:"json",
    formatversion:"2",
    redirects:"1",
  ...
  return [
    ...
    },
    {
      ...
      decode:decodeParseSource,
    },
  ];
}

async function fetchWiktionaryPageSource(inputWord, options={}){
  const word=normalizeLookupTerm(inputWord);
  const headers={
    "Accept":"application/json",
    "User-Agent":"LexiFlow/0.8.7 (+https://github.com/lineana-lyu/lexiflow; etymology)",
    ...(options.headers || {}),
  };
  const attempts=[];

  for(const strategy of wiktionarySourceStrategies(word)){
    try{
      const payload=await fetchJsonWithTimeout(
        WIKTIONARY_API+"?"+strategy.params.toString(),
        {...options,headers}
      );
      const decoded=strategy.decode(payload);
      attempts.push({strategy:strategy.id,state:decoded.state});
      if(decoded.state==="source")return {word,text:decoded.text,strategy:strategy.id,attempts};
      if(decoded.state==="missing")return {word,text:"",strategy:strategy.id,attempts,missing:true};
    }catch(err){
      attempts.push({
        strategy:strategy.id,
        state:"failed",
        code:clean(err?.code) || "ETYMOLOGY_PROVIDER_FAILED",
      });
    }
  }

  const unavailable=new Error("Wiktionary page source unavailable");
  unavailable.code=attempts.some(item=>item.state==="failed")
    ? "ETYMOLOGY_WIKTIONARY_UNAVAILABLE"
    : "ETYMOLOGY_WIKTIONARY_RESPONSE_INVALID";
...
async function fetchWiktionaryEvidence(inputWord, options = {}) {
  ...
  const sourceResult=await fetchWiktionaryPageSource(word,options);
  if(sourceResult.missing || !sourceResult.text)return null;

  const section=languageSection(sourceResult.text, languageCode);
  ...
  return {
    ...
    transport:sourceResult.strategy,
    facts,
    relations:[
      ...extractEtymologyRelations(sections),
      ...extractFormRelations(section),
    ],
  };
}
