...
function coreEvidenceForCanonical(canonicalWord, query = "") {
  ...
  return coreLexicon.lookupExact(canonical, "primary", {
    sourceQuery:clean(query),
    normalizedQuery:clean(query),
    autoResolved:Boolean(query),
...
function trustedChineseCandidates(query, preferredWord = "") {
  ...
  const core = coreLexicon.chineseCandidates(q, 12).map(item => {
    ...
    const semantic = coreLexicon.lookupChineseWord(identity.canonicalWord, q);
    return {
      ...item,
      ...identity,
      ...
      semanticResult:semantic?.chineseSenseMatched === true ? semantic : null,
      semanticMatched:semantic?.chineseSenseMatched === true,
  ...
  const local = ecdict.searchChinese(q, 8).map(item => {
    ...
    const evidence = coreEvidenceForCanonical(identity.canonicalWord, q);
    const coreSemantic = coreLexicon.lookupChineseWord(identity.canonicalWord, q);
    const semantic = coreSemantic?.chineseSenseMatched === true
      ? coreSemantic
      : (item.result?.chineseSenseMatched === true ? item.result : null);
    return {
      ...identity,
      ...
      semanticMatched:Boolean(semantic),
