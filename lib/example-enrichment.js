...
function normalizeGenerated(word, senses, parsed) {
  ...
  const out = [];
  const rows = Array.isArray(parsed) ? parsed : Array.isArray(parsed?.senses) ? parsed.senses : Array.isArray(parsed?.translations) ? parsed.translations : [];
...
async function translateExamplesNow(word, senses) {
  ...
  const stdout = await runCodex(prompt, 28000);
  const parsed = extractJson(stdout);
  if (!parsed) return [];
