...
function diagnosticSourceRank(source) {
  const value = String(source || "");
  if (value.startsWith("mechanics")) return 30;
  if (value.startsWith("issue")) return 20;
  if (value.startsWith("change")) return 10;
  return 0;
}

function sameDiagnosticRange(a, b) {
  return Boolean(
    a?._range && b?._range &&
...
function sameReplacement(a, b) {
  return String(a?.replacement || "").trim() === String(b?.replacement || "").trim();
}
...
function choosePrimaryDiagnostic(current, candidate) {
  ...
  if (sameDiagnosticRange(current, candidate)) {
    const currentSource = String(current._source || "");
    const candidateSource = String(candidate._source || "");

    // Deterministic mechanics are authoritative for the exact surface rule
    // they validate. Model prose must not be concatenated onto that reason.
    if (currentSource.startsWith("mechanics") || candidateSource.startsWith("mechanics")) {
      return currentSource.startsWith("mechanics") ? current : candidate;
    }

    // If issue/review-summary disagree on the concrete replacement for the
    // same span, the change that belongs to the final suggestion is the single
    // replacement authority. Keeping both creates contradictory UI.
    if (!sameReplacement(current, candidate)) {
      if (currentSource.startsWith("change")) return current;
      if (candidateSource.startsWith("change")) return candidate;
    }

    // For the same edit, keep the richer issue explanation over a duplicate
    // change record. Never concatenate equivalent reasons.
    const currentRank = diagnosticSourceRank(currentSource);
    const candidateRank = diagnosticSourceRank(candidateSource);
    if (currentRank !== candidateRank) return currentRank > candidateRank ? current : candidate;
    return current;
  }

  const currentRank = diagnosticSourceRank(current._source);
  const candidateRank = diagnosticSourceRank(candidate._source);
  if (currentRank !== candidateRank) return currentRank > candidateRank ? current : candidate;

  // Independent diagnostics should already be non-overlapping. If an invalid
  // overlap survives, prefer the smaller actionable span rather than letting a
  // broad model rewrite swallow a local verified issue.
