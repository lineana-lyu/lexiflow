...
function issueEvidence(item, targetWord) {
  ...
  if (category === "spelling") {
    const spelling = verifySpellingCorrection({ span, replacement });
...
function normalizeDiagnostic(sentence, item, sourceKind, targetWord) {
  const span = String(item?.span || "").trim();
  const replacement = String(item?.replacement || "").trim();
  ...
  const evidence = issueEvidence(item, targetWord);
...
function normalizeSentenceDiagnostics({ sentence, issues = [], targetWord = "" } = {}) {
  const source = String(sentence || "");
  const candidates = (Array.isArray(issues) ? issues : [])
    .map(item => normalizeDiagnostic(source, item, "issue", targetWord))
    .filter(Boolean);
