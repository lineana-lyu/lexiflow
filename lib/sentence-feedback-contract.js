...
function normalizeSentenceDiagnostics({ sentence, issues = [], targetWord = "", spellingVerifier = verifySpellingCorrection } = {}) {
  ...
  const instructional = merged.filter(item => !item.blocking && item.category === "pronoun_case");

  if (blocking.length) {
    const remainingSlots = Math.max(0, 3 - blocking.length);
    return [...blocking, ...instructional.slice(0, remainingSlots)]
      .map(({ _range, _source, ...item }) => item);
  }

  // With no correctness blocker, prefer one deterministic, high-confidence
  // learning reminder over generative style advice. This keeps the review
  // useful without turning Apply into a proofreading surface.
  const optional = instructional.length
    ? instructional.slice(0, 1)
    : merged.filter(item => !item.blocking).slice(0, 1);
  return optional.map(({ _range, _source, ...item }) => item);
}
