...
function extractMentionPlusRelations(section){
  ...
  for(let i=0;i<matches.length-1;i++){
    ...
    const languageCode=clean(left[1]).toLowerCase();
    if(languageCode!==clean(right[1]).toLowerCase())continue;
    const forms=[normalizeSurface(left[2]),normalizeSurface(right[2])].filter(Boolean);
