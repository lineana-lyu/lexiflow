...
const ANCESTOR_TEMPLATE_TYPES = Object.freeze({
  ...
  cal:"calque_from",
  calque:"calque_from",
  pcal:"partial_calque_from",
  "partial calque":"partial_calque_from",
});
...
function extractRelatedMentionRelations(section){
  const relations=[];
  const seen=new Set();
  const navigableNames=new Set(["m","mention","l","link"]);

  for(const rawTemplate of extractBalancedTemplates(section)){
    const template=parseTemplate(rawTemplate);
    if(!navigableNames.has(template.name))continue;
...
function extractEtymologyRelations(rawSections){
  ...
  for(const section of Array.isArray(rawSections)?rawSections:[]){
    ...
    candidates.push(...extractRelatedMentionRelations(section));
...
module.exports={
  ...
  extractRelatedMentionRelations,
