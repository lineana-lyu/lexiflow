...
const EXCHANGE_ORDER = new Map(["0","3","i","p","d","s","r","t"].map((type,index)=>[type,index]));
...
function parseExchange(value) {
  ...
  for (const part of String(value || "").split("/")) {
    ...
    const form = part.slice(index + 1).trim();
    const label = EXCHANGE_LABELS.get(type);
    if (!label || !form) continue;
    const key = `${type}:${form.toLowerCase()}`;
    if (seen.has(key)) continue;
    seen.add(key);
    forms.push({ type, label, form });
  }
  return forms.sort((a,b)=>(EXCHANGE_ORDER.get(a.type)??99)-(EXCHANGE_ORDER.get(b.type)??99));
}
...
function normalizePhonetic(value) {
  ...
  for (const candidate of candidates) {
    ...
    if (!/[A-Za-z\u0250-\u02AF\u1D00-\u1DBF]/u.test(cleaned)) continue;
    return `/${cleaned}/`;
  }
  return "";
}
...
function chineseTranslationCandidates(value) {
  const raw = String(value || "")
    .replace(/\\r\\n/g, "\n")
    .replace(/\\n/g, "\n")
    .replace(/\\r/g, "\n");
  ...
  for (const line of raw.split(/\r?\n/).map(x => x.trim()).filter(Boolean)) {
    const cleaned = line
      ...
      .replace(/^\s*[①②③④⑤⑥⑦⑧⑨⑩]\s*/, "")
      .trim();
    for (const piece of cleaned.split(/[；;、，,]/).map(x => x.trim()).filter(Boolean)) {
...
function matchedChineseMeaning(value, query) {
  const q = String(query || "").trim();
  if (!q) return null;
  const candidates = chineseTranslationCandidates(value);
  const exact = candidates.find(item => item === q);
  if (exact) return { meaning:q, gloss:exact, exact:true };
  const contained = candidates.find(item => item.includes(q));
  if (contained) return { meaning:q, gloss:contained, exact:false };
  return null;
}
...
function rowToResult(row, mode = "primary", options = {}) {
  ...
  return {
    ...
    wordForms: parseExchange(row.exchange),
...
function rowToChineseResult(row, query, options = {}) {
  const base = rowToResult(row, "primary", {
    ...options,
  ...
  if (!base) return null;
  const match = matchedChineseMeaning(row.translation, query);
  if (!match) return { ...base, chineseSenseMatched:false };
  const sense = base.senses?.[0] || {
    id:makeSenseId(row.word, 0),
    pos:resolvedPos(row, 0),
    exampleEn:"",
    exampleZh:"",
    commonForLearner:true,
    translationConfidence:1,
    senseIntentEn:definitions(row.definition)[0] || "",
    avoidVisualEn:[],
  };
  return {
    ...base,
    ...
    senses:[{
      ...sense,
      meaningZh:match.meaning,
      ...(match.gloss !== match.meaning ? { glossZh:match.gloss } : {}),
    }],
  };
}
...
function searchChinese(query, limit = 8) {
  ...
  try {
    ...
    return rows
      ...
      .map(item => ({
        score: item.score,
        result: rowToChineseResult(item.row, q),
      }));
  } catch (err) {
...
module.exports = {
  ...
  normalizePhonetic,
  parseExchange,
  chineseTranslationCandidates,
  matchedChineseMeaning,
};