...
function ensureNode(graph,{kind,term,display,languageCode,depth,role=""}){
...
function applyAuthority(node,provider=authorityForForm){
  if(node.kind!=="component")return;
  const matches=provider(node.display) || [];
  node.authority=matches.slice(0,4);
}
...
async function buildEvidenceGraph({
  ...
  while(queue.length && providerCalls<maxProviderCalls){
    ...
    applyAuthority(node,authorityProvider);
  ...
  for(const node of graph.nodes.values())applyAuthority(node,authorityProvider);
